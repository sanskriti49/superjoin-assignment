		...
		<div>
			<div className="page-head">
				<div>
					<h2>Overview</h2>
					<p>
						What the system has read, what it took from it, and where those
						readings agree or disagree with each other.
					</p>
				</div>
			</div>
			...
			{stats.loading?.running && (
				<p className="note">
					Reading the starter documents: {stats.loading.loaded} of{" "}
					{stats.loading.total} done
					{stats.loading.current ? `, currently ${stats.loading.current}` : ""}.
					Figures below grow as it goes.
				</p>
			)}
							...
							<tr key={kind}>
								...
								<td>
									<span
										...
										data-kind={kind}
										style={{
											width: `${((counts[kind] || 0) / widest) * 100}%`,
										}}
									/>
								</td>
							</tr>
						...
						{!documents.length && (
							<tr>
								<td colSpan={5} className="muted">
									Nothing loaded yet. Drop a PDF into the panel on the left, or
									drop two that cover the same subject to see them compared.
								</td>
							</tr>
						)}
					</tbody>
			...
			{schema && (
				<>
					...
					<p className="prose muted">
						No list of metrics is built into the system. Each name below exists
						because some document used those words.{" "}
						{formatNumber(schema.total_predicates)} names have appeared so far,{" "}
						{formatNumber(schema.shared_across_documents)} of them in more than
						one document, which is where comparison becomes possible.
					</p>
					<div className="scroller">
						<table>
							<thead>
